...
class LensSaleOrderPage extends StatefulWidget {
  const LensSaleOrderPage({super.key});

  @override
  State<LensSaleOrderPage> createState() => _LensSaleOrderPageState();
}

class _LensSaleOrderPageState extends State<LensSaleOrderPage> {
  String _searchQuery = '';
  DateTime? _fromDate;
  DateTime? _toDate;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<LensSaleOrderProvider>().fetchAllOrders();
    });
  }

  List<LensSaleOrderModel> get _filteredOrders {
    final provider = context.watch<LensSaleOrderProvider>();
    return provider.orders.where((order) {
      if (_searchQuery.isNotEmpty) {
        final q = _searchQuery.toLowerCase();
        if (!(order.billData.billNo.toLowerCase().contains(q) ||
              order.partyData.partyAccount.toLowerCase().contains(q))) {
          return false;
        }
      }
    ...
    return Row(
      ...
      children: [
        Column(
          ...
          children: const [
            Text('Lens Sale Orders', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
            SizedBox(height: 4),
            Text('Manage and track wholesale lens sales orders', style: TextStyle(fontSize: 14, color: Color(0xFF64748B))),
          ],
        ),
        ElevatedButton.icon(
          onPressed: () => context.push('/sales/add-lens-sale-order'),
                  ...
                  SingleChildScrollView(
                    ...
                    child: DataTable(
                      ...
                      columns: const [
                        ...
                        DataColumn(label: Text('TOTAL QTY', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Color(0xFF64748B)))),
                      ...
                      rows: _filteredOrders.map((order) {
                        return DataRow(
                          cells: [
                            ...
                            DataCell(Text('₹ ${order.netAmount.toStringAsFixed(2)}', style: const TextStyle(fontSize: 13, color: Color(0xFF16A34A), fontWeight: FontWeight.bold))),
                              ...
                              Row(
                                ...
                                children: [
                                  ...
                                  IconButton(
                                    ...
                                    onPressed: () => context.push('/sales/add-lens-sale-order?id=${order.id}'),
                                  ),
                                  IconButton(
                                    icon: const Icon(LucideIcons.trash2, size: 18, color: Colors.red),
                                    onPressed: () {
                                      context.read<LensSaleOrderProvider>().deleteOrder(order.id!);
                                    },
                                  ),
                                ],
                              ),
