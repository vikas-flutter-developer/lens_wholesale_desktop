...
class LensSaleInvoicePage extends StatefulWidget {
  const LensSaleInvoicePage({super.key});

  @override
  State<LensSaleInvoicePage> createState() => _LensSaleInvoicePageState();
}

class _LensSaleInvoicePageState extends State<LensSaleInvoicePage> {
  String _searchQuery = '';
  DateTime? _fromDate;
  DateTime? _toDate;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<LensSaleInvoiceProvider>().fetchAllInvoices();
    });
  }

  List<LensSaleInvoiceModel> get _filteredInvoices {
    final provider = context.watch<LensSaleInvoiceProvider>();
    return provider.invoices.where((inv) {
      if (_searchQuery.isNotEmpty) {
        final q = _searchQuery.toLowerCase();
        if (!(inv.billData.billNo.toLowerCase().contains(q) ||
              inv.partyData.partyAccount.toLowerCase().contains(q))) {
          return false;
        }
      }
      if (_fromDate != null && inv.billData.date != null) {
        final d = DateTime.tryParse(inv.billData.date!);
        if (d != null && d.isBefore(_fromDate!)) return false;
      }
      if (_toDate != null && inv.billData.date != null) {
        final d = DateTime.tryParse(inv.billData.date!);
    ...
    return Row(
      ...
      children: [
        Column(
          ...
          children: const [
            Text('Lens Sale Invoices', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
            SizedBox(height: 4),
            Text('Manage final billing entries generated from Challans', style: TextStyle(fontSize: 14, color: Color(0xFF64748B))),
          ],
        ),
        ElevatedButton.icon(
          onPressed: () => context.push('/sales/add-lens-sale-invoice'),
          icon: const Icon(LucideIcons.plus, size: 18),
          label: const Text('Add Sale Invoice'),
            ...
            Expanded(
              ...
              child: TextField(
                decoration: InputDecoration(
                  hintText: 'Search by Invoice No or Party Name...',
                  ...
                  SingleChildScrollView(
                    ...
                    child: DataTable(
                      ...
                      columns: const [
                        ...
                        DataColumn(label: Text('INVOICE NO', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Color(0xFF64748B)))),
                        ...
                        DataColumn(label: Text('DUE AMT', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Color(0xFF64748B)))),
                      ...
                      rows: _filteredInvoices.map((inv) {
                        return DataRow(
                          cells: [
                            DataCell(Text(inv.billData.date ?? '-', style: const TextStyle(fontSize: 13))),
                            DataCell(Text('${inv.billData.billSeries}-${inv.billData.billNo}', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold))),
                            DataCell(Text(inv.partyData.partyAccount, style: const TextStyle(fontSize: 13))),
                            DataCell(Text('₹ ${inv.netAmount.toStringAsFixed(2)}', style: const TextStyle(fontSize: 13, color: Color(0xFF16A34A), fontWeight: FontWeight.bold))),
                            DataCell(Text('₹ ${inv.dueAmount.toStringAsFixed(2)}', style: const TextStyle(fontSize: 13, color: Colors.red, fontWeight: FontWeight.w600))),
                              ...
                              Row(
                                ...
                                children: [
                                  ...
                                  IconButton(
                                    ...
                                    onPressed: () => context.push('/sales/add-lens-sale-invoice?id=${inv.id}'),
                                  ),
                                  IconButton(
                                    icon: const Icon(LucideIcons.trash2, size: 18, color: Colors.red),
                                    onPressed: () {
                                      context.read<LensSaleInvoiceProvider>().deleteInvoice(inv.id!);
                                    },
                                  ),
                                ],
                              ),
