...
class AddRxSalePage extends StatefulWidget {
  final String? saleId;

  const AddRxSalePage({super.key, this.saleId});

  @override
  State<AddRxSalePage> createState() => _AddRxSalePageState();
}

class _AddRxSalePageState extends State<AddRxSalePage> {
  SaleOrderBillData _billData = const SaleOrderBillData(godown: 'Main Store', billSeries: 'RXSI');
  ...
  Future<void> _loadSale() async {
    final provider = context.read<RxSaleProvider>();
    final sale = await provider.fetchSaleById(widget.saleId!);
    if (sale != null && mounted) {
      setState(() {
        _billData = sale.billData;
        _partyData = sale.partyData;
        _items = List.from(sale.items);
        if (_items.length < 5) {
          _items.addAll(List.generate(5 - _items.length, (index) => const RxOrderItem()));
        }
        _taxes = List.from(sale.taxes);
        _paidAmount = sale.paidAmount;
        _remark = sale.remark ?? '';
        _status = sale.status;
        _partyController.text = sale.partyData.partyAccount;
      });
    }
  }
  ...
  void _saveSale() async {
    ...
    final res = widget.saleId != null
        ? await provider.editSale(widget.saleId!, sale.toJson())
        : await provider.createSale(sale.toJson());

    if (res['success'] == true) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('RX Sale saved successfully')));
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          Row(
            children: [
              const Icon(LucideIcons.receipt, color: Color(0xFFEC4899), size: 20),
              const SizedBox(width: 8),
              const Text('Add RX Prescription Sale', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const SizedBox(width: 8),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 2),
                decoration: BoxDecoration(color: const Color(0xFFFDF2F8), borderRadius: BorderRadius.circular(99)),
                child: const Text('RX Invoice', style: TextStyle(color: Color(0xFFDB2777), fontSize: 11, fontWeight: FontWeight.bold)),
              ),
            ],
          ),
          ElevatedButton.icon(
