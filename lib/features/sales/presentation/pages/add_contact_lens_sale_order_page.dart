...
import '../../data/providers/contact_lens_sale_order_provider.dart';
...
import '../widgets/contact_lens_order_grid_widget.dart';

class AddContactLensSaleOrderPage extends StatefulWidget {
  final String? orderId;

  const AddContactLensSaleOrderPage({super.key, this.orderId});

  @override
  State<AddContactLensSaleOrderPage> createState() => _AddContactLensSaleOrderPageState();
}

class _AddContactLensSaleOrderPageState extends State<AddContactLensSaleOrderPage> {
  SaleOrderBillData _billData = const SaleOrderBillData(godown: 'Main Store', billSeries: 'CLSO');
  SaleOrderPartyData _partyData = const SaleOrderPartyData();
  List<ContactLensOrderItem> _items = List.generate(5, (index) => const ContactLensOrderItem());
  ...
  Future<void> _loadOrder() async {
    final provider = context.read<ContactLensSaleOrderProvider>();
    ...
    if (order != null && mounted) {
      setState(() {
        ...
        if (_items.length < 5) {
          _items.addAll(List.generate(5 - _items.length, (index) => const ContactLensOrderItem()));
        }
  ...
  void _addItemsFromBulk(List<ContactLensOrderItem> newMappedItems) {
    setState(() {
  ...
  void _saveOrder() async {
    final provider = context.read<ContactLensSaleOrderProvider>();
    final List<ContactLensOrderItem> filledItems = _items.where((it) => it.itemName.isNotEmpty && it.qty > 0).toList();
    ...
    if (_partyData.creditLimit > 0 && netAmount > _partyData.creditLimit) {
       debugPrint("WARNING: Sale exceeds credit limit of ${_partyData.creditLimit} for ${_partyData.partyAccount}");
       // In future: block save or show validation modal
    }

    final hasInvalidRows = filledItems.any((it) => it.combinationId.isEmpty);
    if (hasInvalidRows) {
       ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Validation Error: One or more rows have invalid SPH/CYL/ADD/EYE combinations. Please fix before saving.')));
       return;
    }

    final order = ContactLensSaleOrderModel(
      id: widget.orderId,
      billData: _billData,
    ...
    if (res['success'] == true) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Contact Lens Order saved successfully')));
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          Row(
            children: [
              const Icon(LucideIcons.fileText, color: Color(0xFF10B981), size: 20),
              const SizedBox(width: 8),
              const Text('Add Contact Lens Sale Order', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const SizedBox(width: 8),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 2),
                decoration: BoxDecoration(color: const Color(0xFFD1FAE5), borderRadius: BorderRadius.circular(99)),
                child: const Text('New', style: TextStyle(color: Color(0xFF047857), fontSize: 11, fontWeight: FontWeight.bold)),
              ),
            ],
          ),
          ElevatedButton.icon(
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Bill Details', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Color(0xFF64748B), letterSpacing: 1)),
