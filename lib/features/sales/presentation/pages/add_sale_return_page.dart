...
class AddSaleReturnPage extends StatefulWidget {
  final String? returnId;

  const AddSaleReturnPage({super.key, this.returnId});

  @override
  State<AddSaleReturnPage> createState() => _AddSaleReturnPageState();
}

class _AddSaleReturnPageState extends State<AddSaleReturnPage> {
  SaleOrderBillData _billData = const SaleOrderBillData(godown: 'Main Store', billSeries: 'SR');
  ...
  Future<void> _loadReturn() async {
    ...
    final sr = await provider.fetchReturnById(widget.returnId!);
    if (sr != null && mounted) {
      setState(() {
  ...
  void _selectParty(dynamic acc) async {
    setState(() {
      ...
      _items = []; // Clear items when party changes
    });
  ...
  void _saveReturn() async {
    ...
    final res = widget.returnId != null
        ? await provider.editReturn(widget.returnId!, sr.toJson())
        : await provider.createReturn(sr.toJson());

    if (res['success'] == true) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Sale Return saved successfully')));
              ...
              child: Column(
                ...
                children: [
                  ...
                  SaleReturnGridWidget(
                    items: _items,
                    customPrices: const {}, // Not strictly needed for returns as prices are from original sales
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          Row(
            children: [
              const Icon(LucideIcons.rotateCcw, color: Color(0xFFEF4444), size: 20),
              const SizedBox(width: 8),
              const Text('Add Sale Return', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const SizedBox(width: 8),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 2),
                decoration: BoxDecoration(color: const Color(0xFFFEE2E2), borderRadius: BorderRadius.circular(99)),
                child: const Text('Return', style: TextStyle(color: Color(0xFFB91C1C), fontSize: 11, fontWeight: FontWeight.bold)),
              ),
            ],
          ),
          ElevatedButton.icon(
      ...
      child: Row(
        ...
        children: [
          ElevatedButton.icon(
            ...
            label: const Text('Save Sale Return'),
