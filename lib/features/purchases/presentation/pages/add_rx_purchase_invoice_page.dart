...
class AddRxPurchaseInvoicePage extends StatefulWidget {
  final PurchaseModel? initialData;
  const AddRxPurchaseInvoicePage({super.key, this.initialData});

  @override
  State<AddRxPurchaseInvoicePage> createState() => _AddRxPurchaseInvoicePageState();
}

class _AddRxPurchaseInvoicePageState extends State<AddRxPurchaseInvoicePage> {
  ...
  Future<void> _handleSave() async {
    ...
    final res = await context.read<PurchaseProvider>().createRxInvoice(_model.toJson());
    if (res['success'] == true) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Rx Purchase Invoice Saved Successfully")));
        context.pop();
      }
    } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(_isEdit ? "Edit Rx Purchase Invoice" : "New Rx Purchase Invoice", 
        ...
        actions: [
          Padding(
            ...
            child: ElevatedButton.icon(
              ...
              label: const Text("Save Rx Invoice"),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Row(children: [Icon(LucideIcons.receipt, size: 18, color: Colors.green), SizedBox(width: 8), Text("Invoice Details", style: TextStyle(fontWeight: FontWeight.bold))]),
            ...
            Row(
              children: [
                ...
                Expanded(child: _buildInputLabel("Bill No", TextField(
                  ...
                  onChanged: (v) => _updateModel(_model.copyWith(billData: _model.billData.copyWith(billNo: v))),
                  decoration: _inputDeco("Invoice #"),
                ))),
                const SizedBox(width: 16),
  ...
  Widget _buildVendorCard() {
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            const Row(children: [Icon(LucideIcons.user, size: 18, color: Colors.green), SizedBox(width: 8), Text("Vendor Details", style: TextStyle(fontWeight: FontWeight.bold))]),
            const SizedBox(height: 20),
          ...
          children: [
            Row(
              ...
              children: [
                ...
                const SizedBox(width: 48),
                _buildSummaryItem("Due Balance", "₹${_model.dueAmount.toStringAsFixed(2)}", color: Colors.orange),
              ],
            ),
          ],
        ),
