...
class AddRxPurchaseReturnPage extends StatefulWidget {
  final PurchaseModel? initialData;
  const AddRxPurchaseReturnPage({super.key, this.initialData});

  @override
  State<AddRxPurchaseReturnPage> createState() => _AddRxPurchaseReturnPageState();
}

class _AddRxPurchaseReturnPageState extends State<AddRxPurchaseReturnPage> {
  ...
  Future<void> _handleSave() async {
    ...
    final res = await context.read<PurchaseProvider>().createRxReturn(_model.toJson());
    if (res['success'] == true) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Rx Purchase Return Saved Successfully")));
        context.pop();
      }
    } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(_isEdit ? "Edit Rx Purchase Return" : "New Rx Purchase Return", 
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Row(children: [Icon(LucideIcons.repeat, size: 18, color: Colors.red), SizedBox(width: 8), Text("Return Details", style: TextStyle(fontWeight: FontWeight.bold))]),
  ...
  Widget _buildVendorCard() {
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            const Row(children: [Icon(LucideIcons.user, size: 18, color: Colors.red), SizedBox(width: 8), Text("Vendor Details", style: TextStyle(fontWeight: FontWeight.bold))]),
            const SizedBox(height: 20),
        ...
        child: Column(
          children: [
            Row(
              ...
              children: [
                _buildSummaryItem("Return Amount", "₹${_model.netAmount.toStringAsFixed(2)}", isBold: true, color: Colors.red),
              ],
            ),
          ],
        ),
