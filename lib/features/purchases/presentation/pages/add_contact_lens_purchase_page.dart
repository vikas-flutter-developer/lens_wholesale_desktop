...
class AddContactLensPurchasePage extends StatefulWidget {
  final PurchaseModel? initialData;
  const AddContactLensPurchasePage({super.key, this.initialData});

  @override
  State<AddContactLensPurchasePage> createState() => _AddContactLensPurchasePageState();
}

class _AddContactLensPurchasePageState extends State<AddContactLensPurchasePage> {
  ...
  Future<void> _handleSave() async {
    ...
    // Check for combinations (Optional validation for now, following React logic)
    final missingInfo = _model.items.any((it) => it.itemName.isNotEmpty && (it.sph.isEmpty || it.eye.isEmpty));
    if (missingInfo) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Sph and Eye (R/L) are required for Contact Lenses")));
      return;
    }

    final res = await context.read<PurchaseProvider>().createCLOrder(_model.toJson());
    if (res['success'] == true) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Contact Lens Purchase Order Saved Successfully")));
        context.pop();
      }
    } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(_isEdit ? "Edit Contact Lens PO" : "New Contact Lens Purchase Order", 
        ...
        actions: [
          Padding(
            ...
            child: ElevatedButton.icon(
              ...
              label: const Text("Save CL Order"),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Row(children: [Icon(LucideIcons.shoppingCart, size: 18, color: Colors.indigo), SizedBox(width: 8), Text("Purchase Order Details", style: TextStyle(fontWeight: FontWeight.bold))]),
            const SizedBox(height: 20),
            Row(
              children: [
                Expanded(child: _buildInputLabel("Series", TextField(
                  ...
                  decoration: _inputDeco("CLPO"),
                ))),
                const SizedBox(width: 16),
                Expanded(child: _buildInputLabel("Order No", TextField(
  ...
  Widget _buildVendorCard() {
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            const Row(children: [Icon(LucideIcons.user, size: 18, color: Colors.indigo), SizedBox(width: 8), Text("Supplier Details", style: TextStyle(fontWeight: FontWeight.bold))]),
            const SizedBox(height: 20),
            Consumer<AccountProvider>(
              builder: (context, accProv, child) {
                final vendors = accProv.accounts;
                return Autocomplete<AccountModel>(
                  ...
                  onSelected: (acc) async {
                    final nextNo = await context.read<PurchaseProvider>().getNextCLPONumber(acc.name);
                  ...
                  fieldViewBuilder: (ctx, ctrl, focus, onFieldSubmitted) {
                    ...
                    return TextField(
                      controller: ctrl,
                      focusNode: focus,
                      decoration: _inputDeco("Search Supplier...", icon: LucideIcons.search),
                    );
                  },
                );
              },
        ...
        child: Column(
          children: [
            Row(
              ...
              children: [
                _buildSummaryItem("Subtotal", "₹${_model.netAmount.toStringAsFixed(2)}", isBold: true),
                const SizedBox(width: 48),
                _buildSummaryItem("Total Due", "₹${_model.dueAmount.toStringAsFixed(2)}", color: Colors.indigo, isBold: true),
              ],
            ),
          ],
        ),
