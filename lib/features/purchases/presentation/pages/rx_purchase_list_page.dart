...
class RxPurchaseListPage extends StatefulWidget {
  const RxPurchaseListPage({super.key});

  @override
  State<RxPurchaseListPage> createState() => _RxPurchaseListPageState();
}

class _RxPurchaseListPageState extends State<RxPurchaseListPage> {
  String _searchQuery = '';
  DateTime? _fromDate;
  DateTime? _toDate;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<PurchaseProvider>().fetchRxOrders();
    });
  }

  List<PurchaseModel> get _filteredOrders {
    final provider = context.watch<PurchaseProvider>();
    return provider.rxOrders.where((order) {
      if (_searchQuery.isNotEmpty) {
        final q = _searchQuery.toLowerCase();
        final party = order.partyData.partyAccount.toLowerCase();
        final billNo = order.billData.billNo.toLowerCase();
        if (!(party.contains(q) || billNo.contains(q))) return false;
      }
    ...
    return Row(
      ...
      children: [
        Column(
          ...
          children: const [
            Text('Rx Purchase Orders', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
            SizedBox(height: 4),
            Text('Track and manage procurement for prescription lenses', style: TextStyle(fontSize: 14, color: Color(0xFF64748B))),
          ],
        ),
        ElevatedButton.icon(
          onPressed: () => context.push('/purchases/add-rx-purchase-order'),
          icon: const Icon(LucideIcons.plus, size: 18),
          label: const Text('New Rx Purchase'),
          style: ElevatedButton.styleFrom(
            backgroundColor: const Color(0xFFC026D3), // Fuchsia for Rx Purchase
            ...
            Expanded(
              ...
              child: TextField(
                decoration: InputDecoration(
                  hintText: 'Search by Order No or Vendor Name...',
    ...
    return Expanded(
      child: Card(
        ...
        child: _filteredOrders.isEmpty 
          ? const Center(child: Text('No Rx purchase orders found.', style: TextStyle(color: Colors.grey)))
          : Scrollbar(
              child: ListView(
                children: [
                  DataTable(
                    ...
                    columns: const [
                      ...
                      DataColumn(label: Text('VENDOR', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Color(0xFF64748B)))),
                    ...
                    rows: _filteredOrders.map((order) {
                      return DataRow(
                        cells: [
                          ...
                          DataCell(Text('${order.billData.billSeries}${order.billData.billNo}', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold))),
                          DataCell(Text(order.partyData.partyAccount, style: const TextStyle(fontSize: 13))),
                          DataCell(Text(order.orderQty.toString(), style: const TextStyle(fontSize: 13))),
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                IconButton(
                                  ...
                                  onPressed: () => context.push('/purchases/add-rx-purchase-order?id=${order.id}'),
                                ),
                                IconButton(
                                  icon: const Icon(LucideIcons.trash2, size: 18, color: Colors.red),
                                  onPressed: () {
                                    context.read<PurchaseProvider>().deletePurchase(order.id!, 'RXPO');
                                  },
                                ),
                              ],
                            ),
