...
import {
  ...
  getRxSale,
  editRxSale,
} from "../controllers/RxSale.contoller";
...
function AddRxSale() {
  ...
  const selectAccount = (acc) => {
    ...
    getNextBillNumberForRxSale(acc.Name).then((nextNo) => {
  ...
  // --- HELPER: getSalePriceForItem(itemOrLens) ---
  const getSalePriceForItem = (item) => {
    if (!item) return 0;

    // 1. Check if custom price exists for this account
  ...
  const handleSave = async () => {
    ...
    if (id) {
      // UPDATE
      res = await editRxSale(id, payload);
  ...
  // WHEN accountWisePrices or allLens changes, update existing items' salePrice where possible
  useEffect(() => {
    if (!accountWisePrices || Object.keys(accountWisePrices).length === 0 || !allLens || allLens.length === 0) return;
    setItems((prev) => {
      let changed = false;
      const updated = prev.map((it) => {
        ...
        if (!lens) return it;
        const newPrice = getSalePriceForItem(lens);
        if (Number(newPrice) !== Number(it.salePrice)) {
          changed = true;
          const qty = parseFloat(it.qty) || 0;
          const disc = Number(it.discount) || 0;
          const totalAmount = roundAmount(qty * newPrice - qty * newPrice * (disc / 100)).toString();
      ...
      <div className="max-w-[1400px] mx-auto p-4 md:p-6 space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-200 pb-4">
          <div>
            ...
            <p className="text-slate-600 text-base font-semibold mt-1 ml-1">
            ...
            <div className="bg-white border border-slate-200 rounded-lg px-4 py-2 w-40 text-right shadow-sm">
              ...
              <div className="font-bold text-slate-700">
                {partyData.creditLimit
                  ? `₹ ${roundAmount(partyData.creditLimit)}`
                  : "0"}
              </div>
            </div>
                    ...
                    <React.Fragment key={item.id}>
                      ...
                      >
                        ...
                        <td className="p-1 bg-blue-50/20 border-l border-blue-100/50">
                          <span className="w-full bg-transparent text-sm p-2 outline-none text-center font-bold text-slate-600 appearance-none block">
                            {item.eye}
                          </span>
                        </td>
                        ...
                        <td className="p-1 bg-blue-50/20 border-r border-blue-100/50 align-top pt-2">
                          <input
                            type="text"
                            placeholder="+0.00"
                            value={item.add}
