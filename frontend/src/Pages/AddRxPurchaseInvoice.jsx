...
function AddRxPurchaseInvoice() {
  ...
  useEffect(() => {
    ...
    const fetchById = async () => {
      const res = await getRxPurchase(id);
      if (res.success) {
  ...
  const selectAccount = (acc) => {
    ...
    getNextBillNumberForRxPurchase(acc.Name).then((nextNo) => {
  ...
  const handleSave = async () => {
    ...
    if (id) {
      // UPDATE
      res = await editRxPurchase(id, payload);

      if (res.success) {
        toast.success("Purchase Invoice updated successfully!");
        navigate("/lenstransaction/purchase/purchaseinvoice?type=rx");
      } else {
        toast.error(res.error || "Update failed");
      }
    } else {
      // CREATE
      res = await addRxPurchase(payload);
  ...
  return (
    <div className="min-h-screen bg-slate-100 pb-20 font-sans selection:bg-blue-100">
      <div className="max-w-[1400px] mx-auto p-4 md:p-6 space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-3 p-1">
          {[
            { label: "Add Order", color: "bg-emerald-600", icon: Plus, onClick: () => navigate("/rxtransaction/rx/AddRxPurchase") },
            { label: "Add RxOrder", color: "bg-orange-600", icon: Plus, onClick: () => navigate("/rxtransaction/rx/AddRxPurchase") },
          ].map((action, idx) => (
            <button
              key={idx}
              ...
              <thead className="bg-slate-50 sticky top-0 z-20 shadow-sm">
                <tr>
                  {[
                    ...
                    { l: "Vendor", w: "min-w-[180px]", align: "center" },
                    { l: "Email", w: "w-16", align: "center" },
                    { l: "Ref Id", w: "w-24", align: "center" },
                    ,
                    { l: "", w: "w-10" },
                  ].map((h, i) => (
                    <th
                      ...
                      >
                        ...
                        <td className="p-1">
                          <input
                            ...
                            value={item.orderNo}
                            onChange={(e) =>
                              updateItem(index, "orderNo", e.target.value)
                            }
                            className="w-full px-2 py-1.5 text-xs text-center bg-transparent border border-transparent focus:bg-white focus:border-blue-400 focus:ring-2 focus:ring-blue-100 rounded outline-none"
                          />
                        </td>
                        ...
                        <td className="p-1">
                          <input
                            ...
                            className="w-full px-2 py-1.5 text-xs text-right bg-transparent border border-transparent focus:bg-white focus:border-blue-400 focus:ring-2 focus:ring-blue-100 rounded outline-none"
                          />
                        </td>
    ...
    "
                        ...
                        {/* Vendor Column - SAME UI AS ITEM DROPDOWN */}
                        <td
                          className="p-1 relative"
                          ref={(el) =>
                            (vendorContainerRefs.current[index] = el)
                          }
                        >
                          <div className="relative">
                            <Search
                              className={`w-3 h-3 absolute left-2 top-2.5 text-slate-400 ${vendorQueries[index] ? "hidden" : "block"
                                }`}
                            />
                            ...
                            <input
                              type="text"
                              value={
                                vendorQueries[index] ?? item.vendorName ?? ""
                              }
                              onChange={(e) => {
                                const val = e.target.value;

                                setVendorQueries((prev) => ({
                                  ...prev,
                                  [index]: val,
                                }));

                                setShowVendorSuggestions((prev) => ({
                                  ...prev,
                                  [index]: true,
                                }));

                                updateItem(index, "vendorName", val);
                              }}
                              onFocus={() =>
                                setShowVendorSuggestions((prev) => ({
                                  ...prev,
                              ...
                              className={`w-full ${vendorQueries[index] ? "pl-2" : "pl-7"
                                } pr-2 py-1.5 text-xs bg-transparent border border-transparent 
        focus:bg-white focus:border-blue-400 focus:ring-2 focus:ring-blue-100 
        rounded transition-all outline-none font-medium text-slate-800 
        placeholder:text-slate-400`}
                              placeholder="Search Vendor..."
                            />

                            {/* MODERN DROPDOWN */}
                            {showVendorSuggestions[index] &&
                              getFilteredVendors(index).length > 0 && (
                                <div className="absolute top-[calc(100%+4px)] left-0 w-[280px] bg-white rounded-lg shadow-xl border border-slate-200 overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-100">
                                  <div className="bg-slate-50 px-3 py-1.5 text-[10px] font-bold text-slate-400 uppercase tracking-wider border-b border-slate-100">
                                    Vendors
                                  </div>
                                  ...
                                  <div className="max-h-[200px] overflow-y-auto">
                                    {getFilteredVendors(index).map((v, i) => (
                                      <div
                                        key={v._id ?? i}
                                        onMouseDown={() =>
                                          selectVendor(v, index)
                                        }
                                        className={`px-3 py-2 cursor-pointer flex flex-col gap-0.5 border-b border-slate-50 last:border-0 ${activeVendorIndexes[index] === i
                                          ? "bg-blue-50"
                                          : "hover:bg-slate-50"
                                          }`}
                                      >
                        ...
                        <td>
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              updateItem(index, "refId", val);
                            }}
                            placeholder="0"
                            className="w-full px-1 py-1.5 text-xs text-center font-mono bg-transparent 
          ...
          <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden flex flex-col">
            ...
            <div className="p-6 flex items-center gap-4">
              ...
              <div className="text-sm text-slate-500">
                Due: ₹{" "}
                {(computeNetAmount() - (Number(paidAmount) || 0)).toFixed(2)}
              </div>
            </div>
...
export default AddRxPurchaseInvoice;
